...
class KVCache:
    def __init__(
        ...
        self.k = mx.zeros(
            (layers, bsz, max_seq_len, kv_heads, head_dim), dtype=mx.bfloat16
        )
        self.v = mx.zeros(
            (layers, bsz, max_seq_len, kv_heads, head_dim), dtype=mx.bfloat16
        )
    ...
    def update(
        ...
        k_layer = mx.concatenate(
            [
                k_new[layer_idx, :, :cur_pos],
                xk,
                k_new[layer_idx, :, cur_pos + seq_len :],
            ],
            axis=1,
        )
        v_layer = mx.concatenate(
            [
                v_new[layer_idx, :, :cur_pos],
                xv,
                v_new[layer_idx, :, cur_pos + seq_len :],
            ],
            axis=1,
        )
